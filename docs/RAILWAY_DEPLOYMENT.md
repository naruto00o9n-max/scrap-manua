...
| `DISCORD_GUILD_ID` | لا (اختياري) | لم يعد مطلوبًا: الأوامر تُسجَّل عالميًا وتعمل في كل السيرفرات. يُستخدم للعرض فقط في سكربت التحقق. |
...
من Discord Developer Portal، افتح التطبيق الذي يطابق `DISCORD_APPLICATION_ID`، وأنشئ أو أعد تعيين توكن البوت ثم احفظه في `DISCORD_BOT_TOKEN`. أضف البوت إلى كل سيرفر تريده باستخدام النطاقين `bot` و`applications.commands` — الأوامر تُسجَّل عالميًا فتظهر في كل السيرفرات التي ينضم إليها البوت دون أي إعداد إضافي.
...
عند تشغيل الخدمة سيعيد البوت تسجيل أوامره **عالميًا** في كل السيرفرات: `/فصل` و`/دمج` و`/مواقع` و`/بحث` و`/نقل` و`/setting` (للمالك فقط) و`/مساعدة`. لا يوجد أمر `/حالة`؛ استخدم لوحة الإدارة للسجل والطابور.
...
### نطاق التوكن مهم — استخدم النطاق الكامل

يجب أن يُولَّد Refresh Token بالنطاق الكامل `https://www.googleapis.com/auth/drive`. التوكن بالنطاق المحدود `drive.file` يجعل البوت يرى المجلدات التي أنشأها هو فقط، حتى المجلدات المشتركة مع حساب البوت بصلاحية «محرر» لا يراها، فيفشل `/نقل` و`/دمج` عليها مع خطأ 404.

أسهل طريقة للتوليد أو التحديث — من جهازك:

```bash
GDRIVE_CLIENT_ID=... GDRIVE_CLIENT_SECRET=... npx tsx scripts/mint-drive-token.mts
```

افتح رابط التفويض بحساب البوت، وافق، ثم ضع التوكن المطبوع في `GDRIVE_REFRESH_TOKEN` في Railway وأعد النشر. وإن كان OAuth Client من نوع Web فأضف `http://localhost:43617` إلى Authorized redirect URIs أولًا.

بعد التحديث يعمل `/نقل` كالمعتاد: البوت ينقل مجلدات الفصول إلى الوجهة المشتركة معه عبر `files.update?addParents` بلا تحميل أو إعادة رفع، مع بقاء معرّفات المجلدات وروابطها كما هي.
...
تُرفع صور الإخراج المدمجة وفق صيغة اللوحة (PNG افتراضيًا). خوارزمية الدمج توزع صفحات الفصل على صور طويلة متساوية الارتفاع عمليًا (سقف افتراضي 15000px قابل للتخصيص من `/الاعدادات`)، وتقص الصفحات الطويلة بكسل-دقيقًا بلا إعادة عيّنة فلا فقدان جودة إضافي.
