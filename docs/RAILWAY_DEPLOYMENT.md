...
أضف MySQL أو خدمة قاعدة بيانات متوافقة مع Drizzle، ثم عرّف `DATABASE_URL` في خدمة التطبيق. لا تضع عنوان قاعدة البيانات داخل الكود أو الأرشيف.

قبل تشغيل البوت، تأكد من وجود جداول المستخدمين والمصادر ومهام الفصول ومحاولات التنفيذ والإعدادات والأدوار والتنبيهات. إذا كانت قاعدة البيانات جديدة، طبّق مخطط Drizzle بالطريقة المناسبة لبيئتك، ثم تحقّق من الجداول قبل إرسال طلبات Discord. لا تستخدم أوامر حذف الجداول في الإنتاج.
...
|---|---:|---|
| `DATABASE_URL` | نعم | اتصال MySQL/TiDB بسجل المستخدمين والطلبات والإعدادات. |
